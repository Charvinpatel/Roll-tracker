...
export default function Dispatches() {
  ...
  const load = async () => {
    try {
      const [dRes, vRes] = await Promise.all([dispatchAPI.getAll(filterVendor), vendorAPI.getAll()]);
  ...
  return (
    <>
      <div className="flex-between">
        ...
        <div className="header-actions-group">
          <div className="search-filter-group">
            ...
            />
            <select className="search-bar" value={filterVendor} onChange={e => setFilterVendor(e.target.value)}>
              <option value="">All Vendors</option>
              {vendors.map(v => <option key={v._id} value={v._id}>{v.name}</option>)}
            </select>
          </div>
          <button className="btn btn-primary" onClick={() => setModal({ type: 'add' })}>+ New Dispatch</button>
        </div>
      </div>
      ...
      <div className="card">
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Vendor</th><th>Quantity</th><th>Dispatch Date</th><th>Notes</th><th>Status</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {dispatches.filter(d => 
                ...
                <tr key={d._id}>
                  ...
                  <td>
                    <button className="date-btn" title="Click to edit date"
                      onClick={() => setModal({ type: 'date', data: { id: d._id, dtype: 'dispatch', date: d.dispatchDate } })}>
                  ...
                  <td>
                    <div className="flex-gap">
                      <button className="btn btn-ghost btn-sm" onClick={() => setModal({ type: 'edit', data: d })}>Edit</button>
                      <button className="btn btn-danger btn-sm" onClick={() => setConfirmDelete(d._id)}>✕</button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          {dispatches.filter(d => 
            ...
            <div key={d._id} className="rc">
              <div className="flex-between" style={{ marginBottom: 6 }}>
                <b>{d.vendor?.name || '—'}</b>
                <span className="badge badge-yellow">{d.quantity} rolls</span>
              </div>
              <div className="flex-between">
                <div style={{ fontSize: 12, color: 'var(--muted)' }}>{fmtDate(d.dispatchDate)}</div>
                <div className="flex-gap">
                  <button className="btn btn-ghost btn-sm" onClick={() => setModal({ type: 'edit', data: d })}>Edit</button>
      ...
      {modal?.type === 'add' && <DispatchModal onClose={() => setModal(null)} onSave={() => { setModal(null); load(); }} />}
      {modal?.type === 'edit' && <DispatchModal dispatch={modal.data} onClose={() => setModal(null)} onSave={() => { setModal(null); load(); }} />}
      {modal?.type === 'date' && (
        <DateEditModal
